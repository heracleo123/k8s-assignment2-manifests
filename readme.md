...
K8s Manifests (Assignment 2):Bashgit remote add origin https://github.com/heracleo123/k8s-assignment2-manifests.git
2. Infrastructure ValidationBashssh -i mykey.pem ec2-user@<YOUR_EC2_IP>
cd ~/k8s-manifests
...
🔄 Phase 5: Rolling Updates (Task 6)1. Sideloading the v1 ImageBashdocker tag 334566130486.dkr.ecr.us-east-1.amazonaws.com/webapp:latest 334566130486.dkr.ecr.us-east-1.amazonaws.com/webapp:v1
kind load docker-image 334566130486.dkr.ecr.us-east-1.amazonaws.com/webapp:v1 --name assignment2-cluster
...
📝 Troubleshooting & Post-MortemIssueResolutionDisk Space ExhaustionCleaned up Docker system images, freeing 2.18GB of space.SCP Permission DeniedSwitched to Git push/pull workflow to maintain commit history.ECR Token ExpirationRegenerated AWS tokens and recreated regcred secrets.Python TypeErrorFixed missing DBPORT environment variable in the deployment manifest.Deployment ScalingResolved label selector conflict by deleting manual ReplicaSets.🧹 CleanupBashkind delete cluster --name assignment2-cluster
